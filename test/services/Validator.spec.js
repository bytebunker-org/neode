...
describe("Services/Validator.js", () => {
			...
			it("should allow a whitelist of values", (done) => {
				...
				Validator(instance, model, { value: "D" })
					...
					.catch((e) => {
						...
						const value = e.details.find((e) =>
							e.path.includes("value"),
						);
		...
		describe("DateTime", () => {
			describe("after", () => {
				it("should accept a date as a minimum value", (done) => {
					...
					Validator(instance, model, {
						date: neo4j.types.DateTime.fromStandardDate(
							new Date("2017-01-01"),
						),
					})
						.then(() => {
			...
			describe("before", () => {
				...
				it("should accept valid string", (done) => {
					...
					Validator(instance, model, {
						...
						.then((res) => {
							expect(res.date).to.be.an.instanceOf(
								neo4j.types.DateTime,
							);
							expect(
								new Date(res.date.toString()).getTime(),
	...
	describe("UUID", () => {
		...
		it("should accept a valid uuidv4", (done) => {
			...
			Validator(instance, model, {
				uuid: "6bfa94dc-6534-4a02-a0ea-ccb5a2275441",
			})
				.then((res) => {
					expect(res.uuid).equal(
						"6bfa94dc-6534-4a02-a0ea-ccb5a2275441",
					);
				})
				.catch((e) => {
					assert(false, e.message);
	...
	describe("Relationships", () => {
		...
		it("should accept a node", (done) => {
			...
			Validator(instance, model, {
				relationship: { prop: "value", node: new Node() },
			})
				.then(() => {
					assert(true);
				})
	...
	describe("Spatial", () => {
		it("should fail if latitude and longitude are passed", (done) => {
			...
			Validator(instance, model, {
				point: { latitude: 37.795972, longitude: -122.407994 },
			})
				.then(() => {
					assert(false, "Should fail validation");
				})
		...
		it("should pass is valid point is passed", (done) => {
			...
			Validator(instance, model, {
				point: new neo4j.types.Point(4326, 37.795972, -122.407994),
			})
				.then(() => done())
				.catch((e) => done(e));
		});
