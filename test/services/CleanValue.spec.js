...
describe("Services/CleanValue.js", () => {
	it("should handle a float", () => {
		const input = "1.2";
		const expected = Number.parseFloat(1.2);
	...
	it("should handle an int", () => {
		const input = "1.2";
		const expected = Number.parseInt(1.2);
	...
	describe("DateTime", () => {
		...
		it("should handle a DateTime as text", () => {
			const input = new Date();
			const output = CleanValue(
				{ type: "datetime" },
				input.toISOString(),
			);
	...
	describe("Time", () => {
		it("should handle a Time", () => {
			...
			expect(output.minute).to.equal(input.getMinutes());
			expect(output.second).to.equal(input.getSeconds());
			expect(output.nanosecond).to.equal(
				input.getMilliseconds() * 1000000,
			);
			// expect(output.timeZoneOffsetSeconds).to.equal(Math.abs(input.getTimezoneOffset()) * -60);
		});
