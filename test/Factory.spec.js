		...
		it("should hydrate a node and eager relationships", (done) => {
            ...
            `)
				...
				.then((res) => {
					...
					expect(
						relationship.startNode().get("id").toNumber(),
					).to.equal(3);
					expect(
						relationship.endNode().get("id").toNumber(),
					).to.equal(4);
					expect(
						relationship.otherNode().get("id").toNumber(),
					).to.equal(4);
					...
					expect(
						node.get("nodes").first().get("id").toNumber(),
					).to.equal(7);
