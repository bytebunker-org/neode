...
import { ERROR_TRANSACTION_FAILED } from "../src/util/TransactionError.js";
...
describe("index.js", () => {
	...
	it("should load modules from a directory", () => {
		...
		expect(
			output.model("ScanDirTest").properties().get("id"),
		).to.be.an.instanceOf(Property);
		expect(
			output.model("ScanDirTest").properties().get("name"),
		).to.be.an.instanceOf(Property);
	});
	...
	it("should run a cypher read query", (done) => {
		instance
			.readCypher("MATCH (n) RETURN count(n)")
			.then((res) => {
				expect(res.records).to.be.an("array");
				expect(res.records.length).to.equal(1);
	...
	it("should handle error in syntax query", (done) => {
		instance.cypher("MATCH (n) RETURN coutn(n)").catch((err) => {
			done();
		});
	});
	...
	describe("::batch", () => {
		...
		it("should throw a transaction error on error", (done) => {
			instance
				.batch(["MATCH (a) RETURN b", "RETURN x"])
				.then(() => {
	...
	describe("::relateTo", () => {
		it("should relate two nodes", (done) => {
			...
			Promise.all([
				...
				.then(([from, to]) => {
					return instance
						.relate(from, to, "relate_test", props)
						.then((rel) => {
				...
				.then((rel) => {
					return instance
						...
						.then((res) => {
							expect(
								res.records[0].get("count").toNumber(),
							).to.equal(1);
						});
				})
				.then(() => done())
		...
		it("should throw an error for an unknown relationship type", (done) => {
			Promise.all([
				...
				return instance
					.relate(from, to, "unknown")
					.then((rel) => {
						assert(
							false,
							"Error should be thrown on unknown relationship type",
						);
					})
					.catch((e) => {
						done();
	...
	describe("::first", () => {
		it("should find a label by a property", (done) => {
			...
			instance
				...
				.then((res) => {
					return instance.first(label, key, value).then((found) => {
	...
	describe("::cypher", () => {
		it("should execute a function as part of a session", (done) => {
			...
			instance
				.cypher("MATCH (n) RETURN COUNT(n)", {}, session)
				.then((res) => {
