...
export function RelateTo(
	...
	return GenerateDefaultValues(neode, relationship, properties)
		.then((properties) =>
			Validator(neode, relationship.schema(), properties),
		)
		.then((properties) => {
			const direction_in =
				relationship.direction() == DIRECTION_IN ? "<" : "";
			const direction_out =
				relationship.direction() == DIRECTION_OUT ? ">" : "";
			...
			return neode.writeCypher(query, params).then((res) => {
				const rel = res.records[0].get("rel");
				const hydrate_from =
					relationship.direction() == DIRECTION_IN ? to : from;
				const hydrate_to =
					relationship.direction() == DIRECTION_IN ? from : to;
