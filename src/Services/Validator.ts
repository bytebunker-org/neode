...
const neoInteger = Joi.extend({
	...
	base: Joi.alternatives().try([
		Joi.number().integer(),
		Joi.object().type(neo4j.types.Integer),
	]),
	name: "integer",
	language: {
		before: "Value before minimum expected value",
	...
	Object.keys(schema).forEach((key) => {
		...
		Object.keys(config).forEach((validator) => {
			...
			if (validator == "regex") {
				...
				ignore.indexOf(validator) == -1 &&
				validation[validator]
			) {
				validation = validation[validator](options);
			} else if (
				ignore.indexOf(validator) == -1 &&
				booleans.indexOf(validator) == -1
			) {
				throw new Error(
 ...
 * @return {Promise}
 */
export function Validator(neode, model, properties) {
	...
	return new Promise((resolve, reject) => {
		Joi.validate(properties, schema, joi_options, (err, validated) => {
			if (err) {
				return reject(
					new ValidationError(err.details, properties, err),
				);
			}
